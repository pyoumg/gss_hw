...
__global__ void Sum_n_elements_Kernel(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
    ...
    int start = index - Nf < 0 ? 0 : index - Nf;
    int end = index + Nf > N-1 ? N-1 : index + Nf;

    int result = 0;
    for (int i = start; i <= end; i++) {
        result += d_ArrayElements[i];
    }
    d_SumOfArrayElements[index] = result;
...
void Sum_n_elements_CPU(IN int *p_ArrayElements, OUT int *p_SumOfElements_CPU, int Nf) {
    ...
    for (int index = 0; index < N; index++) {
        start = index - Nf < 0 ? 0 : index - Nf;
        end = index + Nf > N - 1 ? N - 1 : index + Nf;

        result = 0;
        for (int i = start; i <= end; i++) {
            result += p_ArrayElements[i];
        }
        p_SumOfElements_CPU[index] = result;
    }
}
...
cudaError_t Sum_n_elements_GPU(IN int *p_ArrayElements, OUT int *p_SumOfElements_GPU, int Nf) {
    ...
    CUDA_CALL(cudaMalloc(&in, size))
        CUDA_CALL(cudaMemcpy(in, p_ArrayElements, size, cudaMemcpyHostToDevice))
        CUDA_CALL(cudaMalloc(&out, size))

        dim3 dimBlock(BLOCK_SIZE, 1);
    dim3 dimGrid(N / dimBlock.x, 1);
    CHECK_TIME_INIT_GPU();
    CHECK_TIME_START_GPU();
    Sum_n_elements_Kernel << < dimGrid, dimBlock >> > (in, out,N,Nf);
    CHECK_TIME_END_GPU(device_time);

    CUDA_CALL(cudaDeviceSynchronize())
        CUDA_CALL(cudaMemcpy(p_SumOfElements_GPU, out, size, cudaMemcpyDeviceToHost))
        CHECK_TIME_DEST_GPU();
    return cudaStatus;
}
