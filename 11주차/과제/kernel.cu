...
cudaError_t Fibonacci_GPU(int *h_Fibo_n, int *h_Fibo_v, int *h_Fibo_v_GPU)
{
    ...
    CUDA_CALL(cudaMalloc(&pn, size))
        CUDA_CALL(cudaMemcpy(pn, h_Fibo_n, size, cudaMemcpyHostToDevice))
        CUDA_CALL(cudaMalloc(&pv, size))
        // Assume that width and height are multiples of BLOCK SIZE.
        dim3 dimBlock(BLOCK_SIZE, 1);
    dim3 dimGrid(N / dimBlock.x, 1);
    CHECK_TIME_START_GPU();
    Fibonacci_Kernel << < dimGrid, dimBlock >> > (pn,pv);
    CHECK_TIME_END_GPU(device_time);


    CUDA_CALL(cudaDeviceSynchronize())
        CUDA_CALL(cudaMemcpy(h_Fibo_v_GPU, pv, size, cudaMemcpyDeviceToHost))
    CHECK_TIME_DEST_GPU();
    return cudaStatus;
}