...
void Mat_Vec_Multiply_GPU(float* p_VecX, float* p_MatA, float* p_VecY_G) {
	...
	float* vec_x, * mat_a, * vec_y;
	float size = n * ELEM_PER_VECTOR * sizeof(float);
	cudaError_t cudaStatus;
	// Choose which GPU to run on, change this on a multi-GPU system.
	cudaStatus = cudaSetDevice(0);
	if (cudaStatus != cudaSuccess) {
	...
	CUDA_CALL(cudaMalloc(&vec_x, size))

	CUDA_CALL(cudaMemcpy(vec_x, p_VecX, size, cudaMemcpyHostToDevice))

	CUDA_CALL(cudaMalloc(&vec_y, size))

	size = ELEM_PER_VECTOR * ELEM_PER_VECTOR * sizeof(float);
	CUDA_CALL(cudaMalloc(&mat_a, size))
	CUDA_CALL(cudaMemcpy(mat_a, p_MatA, size, cudaMemcpyHostToDevice))
	...
	CHECK_TIME_INIT_GPU()
	CHECK_TIME_START_GPU()
	Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (vec_y, vec_x, mat_a, ELEM_PER_VECTOR);
	CHECK_TIME_END_GPU(device_time)
	CHECK_TIME_DEST_GPU()
	//CUDA_CALL(cudaGetLastError())
	...
	CUDA_CALL(cudaDeviceSynchronize())
	CUDA_CALL(cudaMemcpy(p_VecY_G, vec_y, size, cudaMemcpyDeviceToHost))
